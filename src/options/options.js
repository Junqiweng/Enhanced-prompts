...
function setupEventListeners() {
    ...
    // 模型列表刷新功能
    document.getElementById('refresh-grok-models').addEventListener('click', fetchGrokModels);
    document.getElementById('refresh-claude-models').addEventListener('click', fetchClaudeModels);
    document.getElementById('refresh-gemini-models').addEventListener('click', fetchGeminiModels);
}
    ...
    try {
        ...
        switch(modelType) {
            ...
            case MODEL_NAME_GEMINI:
                ...
                modelVariant = Elements.geminiModel.value;
                // Construct the URL dynamically based on the selected model
                url = `https://generativelanguage.googleapis.com/v1beta/models/${modelVariant}:generateContent?key=${apiKey}`;
                bodyPayload = {
...
// 从XAI获取Grok模型列表
function fetchGrokModels() {
    // 保存当前选中的模型以便刷新后保持选择
    const currentModel = Elements.grokModel.value;
    
    // 显示加载状态
    const btn = document.getElementById('refresh-grok-models');
    ...
    .then(data => {
        ...
        if (data && data.data && Array.isArray(data.data)) {
            ...
            // 恢复之前选择的模型（如果存在）
            if (Array.from(Elements.grokModel.options).some(opt => opt.value === currentModel)) {
                Elements.grokModel.value = currentModel;
            }
            
            showStatus('成功获取Grok模型列表', 'success');
        } else {
    ...
    .catch(error => {
        ...
        // 添加默认模型选项
        const defaultModels = ['grok-3-beta', 'grok-3-mini-beta'];
...
// 从Anthropic获取Claude模型列表
function fetchClaudeModels() {
    // 保存当前选中的模型以便刷新后保持选择
    const currentModel = Elements.claudeModel.value;
    
    // 显示加载状态
    const btn = document.getElementById('refresh-claude-models');
    const originalText = btn.textContent;
    btn.textContent = '正在加载...';
    btn.disabled = true;
    ...
    fetch('https://api.anthropic.com/v1/models', {
        method: 'GET',
        headers: {
            'x-api-key': Elements.claudeKey.value,
    ...
    .then(data => {
        // 清空当前选项
        Elements.claudeModel.innerHTML = '';
        ...
        if (data && data.models && Array.isArray(data.models)) {
            ...
            // 恢复之前选择的模型（如果存在）
            if (Array.from(Elements.claudeModel.options).some(opt => opt.value === currentModel)) {
                Elements.claudeModel.value = currentModel;
            }
            
            showStatus('成功获取Claude模型列表', 'success');
        } else {
            throw new Error('无效的模型数据格式');
        }
    })
...
function fetchGeminiModels() {
    // 保存当前选中的模型以便刷新后保持选择
    const currentModel = Elements.geminiModel.value;
    
    // 显示加载状态
    const btn = document.getElementById('refresh-gemini-models');
    const originalText = btn.textContent;
    btn.textContent = '正在加载...';
    btn.disabled = true;
    
    // 这里应该是实际API调用来获取模型列表
    // Google AI API可能不直接提供模型列表的端点，这里是模拟实现
    // 实际应用中需要根据Google AI API的文档实现
    fetch(`https://generativelanguage.googleapis.com/v1beta/models?key=${Elements.geminiKey.value}`)
    .then(response => {
        if (!response.ok) {
            throw new Error(`HTTP error! Status: ${response.status}`);
        }
    ...
    .then(data => {
        ...
        if (data && data.models && Array.isArray(data.models)) {
            ...
            // 恢复之前选择的模型（如果存在）
            if (Array.from(Elements.geminiModel.options).some(opt => opt.value === currentModel)) {
                Elements.geminiModel.value = currentModel;
            }
            
            showStatus('成功获取Gemini模型列表', 'success');
        } else {
            throw new Error('无效的模型数据格式');
        }
    })
